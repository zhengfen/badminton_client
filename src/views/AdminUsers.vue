        ...
        <tbody>
          <tr v-for="(item, index) in items" :key="item.id">
            ...
            <td @click="show_team_modal(item)">
              <div class="pointer" v-for="team in item.teams" :key="team.id">
                {{ team.team.name }} -
                {{ trans(getLevelNameById(team.team.level)) }}
                <span v-if="team.position">({{ 
                  trans(getPositionNameById(team.position))
                }})</span>
              </div>
            </td>
            <td>{{ item.licence }}</td>
